...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
  	...
  	#pragma omp parallel
	{
			...
			for (int i = my_number; i < ARRAY_SIZE; i += num_of_threads) {
					z[i] = x[i] + y[i];
			}
	}
}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
	...
	{
			int num_of_threads = omp_get_num_threads();
			int per_share = ARRAY_SIZE / num_of_threads;
			int my_number = omp_get_thread_num();
			for (int i = my_number * per_share; i < (my_number * per_share + per_share); i++) {
					z[i] = x[i] + y[i];
			}
			if (my_number == num_of_threads - 1) {
					for (int i = (my_number * per_share + per_share); i < ARRAY_SIZE; i++) {
							z[i] = x[i] + y[i];
					}
			}
	}
