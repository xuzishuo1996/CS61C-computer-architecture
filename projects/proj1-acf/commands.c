...
void handle_set_nickname (char **args, unsigned count, unsigned n) {
  ...
  if (count != 2) { //if the number of argument is not 2
      handle_invalid_arguments ("show_all_statuses", n);
      return;
  } else if (!istaken_name(args[0]) ) {  //if the argument is not an actual name
      handle_invalid_arguments ("show_all_statuses", n);
      return;
  } else if (!isvalidname(args[1])) { //if the name has been taken as the actual name or nickname by another user
      handle_invalid_arguments ("show_all_statuses", n);
      return;
  } else {  /* IMPLEMENT THE CORE FUNCTIONALITY */
...
void handle_clear_nickname (char **args, unsigned count, unsigned n) {
	/* HANDLE ANY POSSIBLE ERROR CONDITIONS */
  if (count != 1) { //if the number of argument is not 1
      handle_invalid_arguments ("show_all_statuses", n);
      return;
  } else if (!istaken_name(args[0]) ) {  //if the argument is not an actual name
      handle_invalid_arguments ("show_all_statuses", n);
      ...
      struct user_info *user = find_user(args[0]);
      free(*(user->nickname));
      *(user->nickname) = (char *)malloc(strlen(user->name_info->name) + 1);
      strcpy(*(user->nickname), user->name_info->name);
...
void handle_rename (char **args, unsigned count, unsigned n) {
  ...
  if (count != 1) { //if the number of argument is not 1
      handle_invalid_arguments ("show_all_statuses", n);
      return;
  } else if (!isvalidname(args[0]) ) {  //if the name has already been taken as an actual name or a nickname
      handle_invalid_arguments ("show_all_statuses", n);
      return;
  } else {  /* IMPLEMENT THE CORE FUNCTIONALITY */
      ...
      char *old_name = (char *)malloc(strlen(user->name_info->name) + 1);
      strcpy(old_name, user->name_info->name);

      if (strcmp(user->name_info->name, *(user->nickname)) == 0) {  //if the user doesn't have a nickname
          free(*(user->nickname));
          *(user->nickname) = (char *)malloc(strlen(name) + 1);
          strcpy(*(user->nickname), name);
      }

      free(user->name_info->name);
      user->name_info->name = (char *)malloc(strlen(name) + 1);
      strcpy(user->name_info->name, name);
 ...
 * command. */
void handle_mute (char **args, unsigned count, unsigned n) {  //Can nicknames be muted or not?
	/* HANDLE ERROR CONDITIONS */
  if (count != 1) { //if the number of argument is not 1
      handle_invalid_arguments ("show_all_statuses", n);
      return;
  } else if (!istaken_name(args[0])) {  //if the argument is not a name(The name must be an actual name of a user.)
      handle_invalid_arguments ("show_all_statuses", n);
      return;
  } else if (strcmp(*args, users[n]->name_info->name) == 0) { //if the client tries to mute itself
      handle_invalid_arguments ("show_all_statuses", n);
      return;
  } else {  /* IMPLEMENT THE CORE FUNCTIONALITY */
      //get the user_info of the name
      struct user_info *user = find_user(args[0]);
      if (!ismuted(users[n], user)) {
          /* Handle the muter. */
          if(users[n]->muted_total == users[n]->muted_capacity) {
              return; //or output some message?
          }
          users[n]->muted[users[n]->muted_total] = create_name_info(args[0]);
          users[n]->muted_total++;

          /* Handle the mutee. */
          user->name_info->total_tracking -= 1;
      }
...
void handle_unmute (char **args, unsigned count, unsigned n) {
   ...
   if (count != 1) { //if the number of argument is not 1
        ...
        handle_invalid_arguments ("show_all_statuses", n);
        return;
   } else if (strcmp(args[0], users[n]->name_info->name) == 0) { //if the client tries to unmute itself
        handle_invalid_arguments ("show_all_statuses", n);
        return;
   } else {  /* IMPLEMENT THE CORE FUNCTIONALITY */
        //get the user_info of the name
        ...
        if (ismuted(users[n], user)) {
            /* Handle the unmuter. */
            free(users[n]->muted[users[n]->muted_total - 1]);
            users[n]->muted_total--;
            /* Handle the unmutee. */
            user->name_info->total_tracking += 1;
        }
 ...
 * characters, this has already been checked for you. */
 //output_user_status in command_utils.c is a helper.
