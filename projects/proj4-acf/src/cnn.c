 ...
 */
 ...
 void net_classify_cats(network_t* net, vol_t** input, double* output, int n) {
   int share = 8;
   for (int i = 0; i < n / share * share; i += share) {
     batch_t* batch;
     /*#pragma omp parallel
     {
       batch = make_batch(net, share);
       //printf("1\n");
     }*/
     batch = make_batch(net, share);
     ...
     for (int j = 0; j < share; j++) {
       ...
       //printf("2\n");
       //net_forward(net, batch, 0, n);
       //output[i] = batch[11][i]->w[CAT_LABEL];
     }
     #pragma omp parallel for
     for (int j = 0; j < share; j++) {
       //printf("2.5\n");
       net_forward(net, batch, j, j);
       //printf("3\n");
     }
     #pragma omp parallel for
     for (int j = 0; j < share; j++) {
   ...
   //printf("come to here\n");
   int remain = n - (n / share * share);
   batch_t* batch;
   if (remain != 0) {
     /*#pragma omp parallel
     {
       batch = make_batch(net, remain);
       //printf("6\n");
     }*/
     batch = make_batch(net, remain);
     #pragma omp parallel for
     for (int j = 0; j < remain; j++) {
       copy_vol(batch[0][j], input[n / share * share + j]);
       //printf("7\n");
       //net_forward(net, batch, 0, n);
       //output[i] = batch[11][i]->w[CAT_LABEL];
     }
     ...
     #pragma omp parallel for
     for (int j = 0; j < remain; j++) {
       output[n / share * share + j] = batch[11][j]->w[CAT_LABEL];
       //printf("9\n");
     }
     /*#pragma omp parallel
     {
       free_batch(batch, remain);
       //printf("10\n");
     }*/
     free_batch(batch, remain);
   }
